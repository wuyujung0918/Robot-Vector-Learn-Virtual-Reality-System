    ...
    void Update()
    {
        ...
        if (isUpdatingCoordinates)
        {
            ...
            if (targetObject != null)
            {
                ...
                float roundedX = Mathf.Floor(targetPosition.x * 100f) / 10f;
                float roundedY = Mathf.Floor(targetPosition.y * 100f) / 10f;
                float roundedZ = Mathf.Floor(targetPosition.z * 100f) / 10f;

                // 格式化坐标字符串，包括小数点后一位
                string coordinatesText = $" : ({roundedX:F1}, {roundedZ:F1}, {roundedY:F1} )";
